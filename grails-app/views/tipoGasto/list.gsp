	...
	<body>
		...
		<div id="list-tipoGasto" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="nome" title="${message(code: 'tipoGasto.nome.label', default: 'Nome')}" />
					
						<th><g:message code="tipoGasto.pai.label" default="Pai" /></th>
				...
				<g:each in="${tipoGastoInstanceList}" status="i" var="tipoGastoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${tipoGastoInstance.id}">${fieldValue(bean: tipoGastoInstance, field: "nome")}</g:link></td>
					
						<td>${fieldValue(bean: tipoGastoInstance, field: "pai")}</td>
			...
			<div class="pagination">
				<g:paginate total="${tipoGastoInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
